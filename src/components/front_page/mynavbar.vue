<template>
    <div class="mynavbar_outer">
      ...
      <div class="row mynavbar container-xl mx-auto">
        ...
        <div class="col-md-7 py-3  ">
          <div class="nav_items">
            <a href="#" v-show="fetched" v-for="item, index of nav_children" @mouseover="nav_item_over(item, index)"
              @mouseleave="nav_item_leave(item, index)" :key="index">{{
                item.title }}</a>
          </div>
        </div>
  ...
  onMounted(() => {
    ...
    resp.then(data => {
      nav_children.value = data.data;
      fetched.value = true;
    })
  });
  
  function nav_item_over(item, index) {
    if (item.title !== '服务中心' && item.title !== '社区') {
      selected.value = index; staying.value = true
    }
  }
  
  function nav_item_leave(item, index) {
    if (item.title !== '服务中心' && item.title !== '社区') {
      staying.value = false
    }
  }

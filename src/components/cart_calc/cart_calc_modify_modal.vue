<template>
    <b-modal id="cart_calc_modal" centered v-model="show_modal">
        <template #header>
            修改收货地址
...
export default {
    ...
    computed: {
        ...mapState(['realname', 'default_phone', 'default_addr', 'cart_calc_modify_modal'])
    },
    watch: {
        ...
        show_modal: {
            handler(newValue, oldValue) {
                if (!newValue) {
                    this.$store.commit('cart_calc_modify_modal', false);
                }
            }
        }
    },
